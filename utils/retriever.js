...
// Paths are derived from .env to stay flexible
const DB_FILE = path.resolve(__dirname, "..", process.env.DATABASE_PATH || "data/sqlite3/lawgpt_vectors.sqlite");
const EXT_FILE = path.resolve(__dirname, "..", process.env.VEC_EXTENSION_PATH || "node_modules/sqlite-vec-windows-x64/vec0.dll");

let db = null;
const sleep = (ms) => new Promise(res => setTimeout(res, ms));

/**
 * Initializes DB and loads the vector engine.
 */
...
/**
 * SHARED: Fetches embeddings with automatic Exponential Backoff (prevents 429 errors).
 */
...
/**
 * SHARED: AI Reranker that filters the top 10 results down to the best 5.
 */
async function rerankWithRetry(query, candidates, retries = 3) {
    ...
    for (let i = 0; i < retries; i++) {
        try {
            ...
            return candidates.slice(0, 5); // Fail gracefully: return results even if rerank hits a limit
        }
    }
}

/**
 * MAIN SEARCH: Uses a SQL JOIN to combine Math (vectors) with Text (documents).
 */
export async function search(q, k = 5, dataset = "all") {
    ...
    try {
        ...
        // Filter by source (e.g., 'constitution' or 'bns') if selected
...
/**
 * DIRECT LOOKUP: Quick Keyword Search.
 */
